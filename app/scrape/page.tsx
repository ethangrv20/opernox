...
import {
  ...
  Clock, ChevronDown, Copy, Check, X, Zap, Star, Phone, MapPinned
} from 'lucide-react';
...
const SCRAPE_TYPES = [
  { id: 'maps' as ScrapeType, label: 'Maps', icon: MapPin, color: '#ef4444', placeholder: 'roofing companies in new york, dentists in chicago...', hint: '30-60s per run · Google Maps real data' },
...
function JobRow({ job, onDelete }: { job: any; onDelete: (id: string) => void }) {
  ...
  const typeColors: Record<string, string> = { maps: '#ef4444', keywords: '#3b82f6', domain: '#22c55e' };
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '12px 14px', gap: '10px', cursor: 'pointer', userSelect: 'none' }} onClick={() => setOpen(v => !v)}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
            <span style={{ fontSize: '0.65rem', color: tc, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{job.type}</span>
            ...
            {job.status === 'running' && (
              <span style={{ fontSize: '0.65rem', color: '#60a5fa' }}>scraping...</span>
            )}
            {job.status === 'error' && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
          {job.status === 'done' && job.results?.length > 0 && (
            <button onClick={() => downloadCSV(job)} style={{ background: 'none', border: '1px solid #27272a', cursor: 'pointer', padding: '4px 8px', borderRadius: '6px', color: '#71717a', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Download size={10} />
              <span style={{ fontSize: '0.65rem' }}>CSV</span>
            </button>
          )}
        ...
        <div style={{ borderTop: '1px solid #27272a', backgroundColor: '#09090b' }}>
          ...
          {job.status === 'done' && !job.results?.length && (
            <div style={{ padding: '20px 14px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '0.7rem', color: '#3f3f46', marginTop: '4px' }}>Try a less competitive keyword or check Google directly.</p>
            </div>
          )}
      ...
      <div style={{ padding: '12px 14px', borderBottom: '1px solid #1a1a1a' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
          <div style={{ flex: 1 }}>
            ...
            {data.address && (
              <p style={{ fontSize: '0.72rem', color: '#a1a1aa', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <MapPinned size={9} style={{ color: '#52525b', flexShrink: 0 }} />{data.address}
              </p>
            )}
          </div>
          {data.phone && (
            <button onClick={() => onCopy(data.phone, `phone-${data.name}`)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', color: '#52525b', flexShrink: 0 }}>
              {copied === `phone-${data.name}` ? <Check size={11} style={{ color: '#4ade80' }} /> : <Phone size={11} />}
            </button>
          )}
        </div>
        {data.phone && (
          <p style={{ fontSize: '0.72rem', color: '#d4d4d8', marginTop: '4px' }}>{data.phone}</p>
        )}
      </div>
    );
  }
          ...
          <div style={{ backgroundColor: '#18181b', borderRadius: '16px', border: '1px solid #27272a', padding: '18px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '12px' }}>
              ...
              <button type="submit" disabled={loading || !input.trim()}
                style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 18px', borderRadius: '10px', fontSize: '0.8rem', fontWeight: 600, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading || !input.trim() ? 0.4 : 1, backgroundColor: active.color, color: 'white', whiteSpace: 'nowrap' }}>
