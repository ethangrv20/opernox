...
# ─── CHANGE 1: Fix GSC amber message ────────────────────────────────────────
...
# ─── CHANGE 2: Fix AdsPower warning (Mentions) ───────────────────────────────
content = content.replace(
...
# ─── CHANGE 3: Fix AdsPower warning (Reviews) ────────────────────────────────
content = content.replace(
...
content = content.replace(
  ...
  // Keyword row expand state (shows rankings + mentions + reviews inline)
...
old_reviews_end = """    } catch (e: any) { setMsg({ type: 'error', text: e.message }); }
    finally { setReviewLoading(false); }
  }, [user]);

  // ─── Load Competitors ────────────────────────────────────────────────────"""
new_reviews_end = """    } catch (e: any) { setMsg({ type: 'error', text: e.message }); }
    finally { setReviewLoading(false); }
  }, [user]);

  // Load rankings for a specific keyword (inline in expanded keyword panel)
  ...
  // ─── Load Competitors ────────────────────────────────────────────────────"""
content = content.replace(old_reviews_end, new_reviews_end)

# ─── CHANGE 6: Only keep Keywords tab ─────────────────────────────────────────
content = content.replace(
    """  const TABS = [
...
# ─── CHANGE 8: Fix keyword row — add Details button + expanded panel ─────────
# OLD end of keyword row: </div> closes the keyword wrapper, then ))} closes map
# NEW: Details button + expanded panel INSIDE the wrapper, then </div> + ))}
content = content.replace(
    ...
    """                    <button onClick={() => deleteKeyword(kw.id)} style={{ background: 'rgba(239,68,68,0.1)', border: 'none', borderRadius: 8, padding: 8, color: '#ef4444', cursor: 'pointer' }}>
                  ...
                  {/* Expanded panel: rankings + mentions + reviews for this keyword */}
                  {expandedKwId === kw.id && (
...
content = content.replace(
    """{/* ── COMPETITORS ───────────────────────────────────────────────────── */}
        {tab === 'competitors' && (""",
    """{/* ── COMPETITORS (removed — use Client Config) ────────────────── */}
        {false && ("""
)
